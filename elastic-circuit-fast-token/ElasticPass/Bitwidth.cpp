...
static unsigned getOutputSize(const OptimizeBitwidth& OB, const ENode* node, const ENode* succ) {
    ...
    case Branch_c: {
        for (int i = 0; i < node->CntrlPreds->size(); i++)

			// AYA: 03/11/2021: added extra condition to count constants as condition!!!
    ...
    case Start_:
        return CONTROL_SIZE;
    case Source_:  // AYA: note: I never use this type so I don't care what it has!!
