...
class LSQBRAM(lsqConfig: LsqConfigs) extends Module {
  ...
  val bbStoreOffsets = Wire(Vec(lsqConfig.fifoDepth_S, UInt(log2Ceil(lsqConfig.fifoDepth_L).W)))
  val bbStorePorts = Wire(Vec(lsqConfig.fifoDepth_S, UInt(max(1, log2Ceil(lsqConfig.numStorePorts)).W)))
  val bbNumStores = Wire(UInt(max(1 ,log2Ceil(min(lsqConfig.numStorePorts, lsqConfig.fifoDepth_S) + 1)).W))
  val storeTail = Wire(UInt(lsqConfig.fifoDepth_S.W))
  val storeHead = Wire(UInt(lsqConfig.fifoDepth_S.W))
  val storeEmpty = Wire(Bool())
  val bbLoadOffsets = Wire(Vec(lsqConfig.fifoDepth_L, UInt(log2Ceil(lsqConfig.fifoDepth_S).W)))
  val bbLoadPorts = Wire(Vec(lsqConfig.fifoDepth_L, UInt(max(1, log2Ceil(lsqConfig.numLoadPorts)).W)))
  val bbNumLoads = Wire(UInt(max(1 ,log2Ceil(min(lsqConfig.numLoadPorts, lsqConfig.fifoDepth_L) + 1)).W))
  ...
  val storeAddressQueue = Wire(Vec(lsqConfig.fifoDepth_S, UInt(lsqConfig.addrWidth.W)))
  val storeDataQueue = Wire(Vec(lsqConfig.fifoDepth_S, UInt(lsqConfig.dataWidth.W)))
  val loadAddressDone = Wire(Vec(lsqConfig.fifoDepth_L, Bool()))
  val loadDataDone = Wire(Vec(lsqConfig.fifoDepth_L, Bool()))
  val loadAddressQueue = Wire(Vec(lsqConfig.fifoDepth_L, UInt(lsqConfig.addrWidth.W)))
