...
class LoadQueueAxi(config: LsqConfigs) extends Module {
  ...
  for (loadEntryIdx <- 0 until config.fifoDepth) {
    ...
    when(addrKnownPReg(loadEntryIdx) && !dataKnownPReg(loadEntryIdx)) {
      when(!bypassInitiated(loadEntryIdx) && !loadInitiated(loadEntryIdx) && !dataKnown(loadEntryIdx)) {
  ...
  /**
    ...
    */
    for (i <- 0 until config.fifoDepth) {
      when(initBits(i)) {
        dataKnown(i) := false.B
      }.elsewhen((io.loadQIdxForDataInValid && io.loadQIdxForDataIn === i.U) || bypassRequest(i)) {
        dataKnown(i) := true.B
      }
    }
