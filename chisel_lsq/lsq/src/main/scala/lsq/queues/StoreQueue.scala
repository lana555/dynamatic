...
class StoreQueue(config: LsqConfigs) extends Module {
  ...
  /**
    ...
    * are no conflicts
    */
  val storeRequest: Bool = addrKnown(head) && dataKnown(head) && !storeCompleted(head) && noConflicts.forall(x => x)
  ...
  for (storePortId <- 0 until config.numStorePorts) {
    ...
    inputAddrPriorityPorts(storePortId) := CyclicPriorityEncoderOH(condVecAddr, head)
    inputDataPriorityPorts(storePortId) := CyclicPriorityEncoderOH(condVecData, head)
  }
