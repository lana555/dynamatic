...
void DFnetlist_Impl::calculateBackEdges()
{
    ForAllChannels(c) {
		...
		// AYA: 25/03/2022: added a condition to consider as a backward edge any edge going to a Merge node and having the src BB with an ID greater than the dst BB! 
        if ( ((getBlockType(bb) == BRANCH) && src >= dst) || (getBlockType(getDstBlock(c)) == MERGE && src >= dst) )
            setBackEdge(c, true);
       	// Lana 22.03.2022
	//else if ((getBlockType(bb) == MERGE && getBlockType(getDstBlock(c)) == MUX) && src >= dst)
	  //  setBackEdge(c, true);
        else
            setBackEdge(c, false); 
    }
